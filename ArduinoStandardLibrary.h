...
class DigitalInput
{
  ...
  public:
    DigitalInput(unsigned int pin, unsigned long threshold)
    {
      ...
      this->oldValue = 0;
      this->isChanged = 0;
      initTimer(&this->activationTimer, threshold);
    }
...
class AnalogInput
{
  private:
    unsigned int pin;
    unsigned int value;
    unsigned int maxSizeFilter;
    unsigned int filterCount;
    void filterValue(int inputValue);

  public:
    AnalogInput(unsigned int pin, unsigned int filterSize)
    {
      pinMode(pin, INPUT);
      this->pin = pin;
      this->value = 0;
      this->maxSizeFilter = filterSize;
      this->filterCount = 0;
    }
